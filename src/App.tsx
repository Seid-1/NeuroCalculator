    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center sm:p-8">
      ...
      <div className="w-full max-w-md bg-slate-900 sm:rounded-3xl shadow-2xl overflow-hidden border-x sm:border border-slate-800 relative flex flex-col h-[100dvh] sm:h-auto sm:min-h-[640px]">
        ...
        <div className="h-14 flex items-center justify-between px-6 bg-slate-900 border-b border-slate-800 z-10 shrink-0">
          ...
          <div className="text-slate-500 text-sm font-semibold tracking-wider">
            NEUROCALC
          </div>
          <button
        ...
        <div
          className={`absolute inset-y-0 right-0 w-full sm:w-72 bg-slate-900/95 backdrop-blur-xl shadow-2xl transform transition-transform duration-300 border-l border-slate-800 z-50 ${
        ...
        >
          <div className="p-4 h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar">
              {history.length === 0 && (
                <p className="text-slate-600 text-sm italic text-center mt-10">
                  No history yet
                </p>
              )}
              {history.map((item) => (
                <button
                  ...
                  onClick={() => {
                    if (!item.expression.startsWith("Photo Analysis")) {
                      ...
                      let exprToLoad = item.expression
                        .replace("AI: ", "")
                        .replace("Plot: ", "")
                        .replace("Graph: ", "");
                ...
                >
                  <div className="text-slate-500 text-xs mb-1 font-mono truncate">
                    {item.expression}
                  </div>
                  <div
